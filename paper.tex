...
While the ILP provides exact solutions, solving it requires branch-and-bound over $O(n^2)$ binary variables, requiring worst-case exponential time. The $O(n^3)$ transitivity constraints further slow each LP relaxation within the search tree, making the approach infeasible for $n > 60$ on modern hardware. This motivates heuristic approaches that sacrifice optimality guarantees for scalability.
...
The ILP is correct by construction: the transitivity constraints ensure $u_{ij} = 1$ whenever $i$ and $j$ remain connected via any path, preventing undercounting.

\paragraph{Formulation size vs.\ solving time.} The formulation has $O(n^2)$ binary variables and $O(n^3)$ transitivity constraints, polynomial to \emph{write down}. However, solving a binary ILP is NP-hard: branch-and-bound explores up to $2^{O(n^2)}$ search tree nodes, each requiring an LP relaxation over the $O(n^3)$ constraints. It is this exponential search, not the polynomial formulation size, that limits the ILP to small $n$. Table~\ref{tab:ilp_scaling} shows the empirical boundary.
...
\begin{enumerate}
    ...
    \item \textbf{Greedy pair reduction.} Iteratively select the node $v^* = \arg\min_{v \in V \setminus S} f(S \cup \{v\})$. At each of $k$ iterations, evaluate all remaining candidates by calling \textsc{Evaluate}. This directly optimizes the objective but makes greedy (myopic) choices and cannot backtrack.
...
The ILP solved karate ($n = 34$) in 0.63s and dolphins ($n = 62$) in 60s; football ($n = 115$) timed out at 180s. The exponential growth in branch-and-bound search time confirms that all 16 benchmark instances ($n > 120$) are beyond the ILP's reach. Subsequent comparisons focus on heuristic methods.
...
MS-ILS \textbf{matches IRMS exactly} on individual components (0\% gap on all 6 runs) and stays within 2.7\% on the full network. On comp0 ($k=61$), MS-ILS is 80$\times$ faster (0.21s vs 16.7s), since IRMS's instance reduction overhead does not pay off on already-sparse graphs.
